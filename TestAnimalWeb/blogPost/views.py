...
class GetUserAllArticle(generics.ListAPIView):  # lấy tất cả bài báo của một user nào đó 
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(user = int(idUser)).order_by('-created_at')
        return queryset
...
######### fixing here 
class FilterArticleFollowingTag (generics.ListAPIView): # lọc các bài viết thuộc lớp đv mà user theo dõi 
    pagination_class = ArticlePagination
    serializer_class = ArticleSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    def get_queryset(self):
        queryClass = UserFollowAnimal.objects.filter(user=self.request.user).values_list('animal', flat=True)  # lấy các loài user quan tâm
        listClass = list(queryClass)
        # sau đó lọc các bài viết theo cái loài đó 
        queryset = Article.objects.all()
        for x in listClass:
            queryset = queryset | Article.objects.filter(typeClass = x)
        return queryset
...
class ArticleListFavorite(generics.ListAPIView): # lấy các bài đăng có nhiều lượt yêu thích nhất
    pagination_class = ArticlePagination
    serializer_class = ArticleSerializer
    permission_classes = (AllowAny,)
    def get_queryset(self):
        queryset = Article.objects.order_by('-like')
        return queryset

class ArticleListTrend(generics.ListAPIView): # lấy các bài đăng có nhiều lượt xem nhất
    pagination_class = ArticlePagination
    serializer_class = ArticleSerializer
    permission_classes = (AllowAny,)
    def get_queryset(self):
        queryset = Article.objects.order_by('-view')
        return queryset
...
class UserFollowingAnimal(generics.CreateAPIView):  # User follow 1 animal's class
    queryset = UserFollowAnimal.objects.all()
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    serializer_class = TagAnimalFollowSerializers
    def get_queryset(self): 
